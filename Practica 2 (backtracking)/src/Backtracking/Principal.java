...
public class Principal {
    ...
    private static int mejorNumMutaciones(String s1, String s2, int pointer,int numMov, int mejorMov ){
        if (s1.equals(s2)){
            return Math.min(numMov, mejorMov);
        }
        else{
            if((pointer < s2.length() && pointer < s1.length()) && s1.charAt(pointer) == s2.charAt(pointer)) //si son iguales los caracteres en la pos pointer los dejamos asi
                ...
                if (pointer < s1.length() && pointer < s2.length()) { //cambiar elem
                    String aux = "";
                ...
                if (s1.length() > s2.length()){ //elimina elem
